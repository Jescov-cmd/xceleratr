...
export default function Settings({ settings, updateSettings }: Props) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await window.api.applyMouse({
        ...
        sensitivityEnabled:      s.sensitivityEnabled,
  ...
  return (
    <div className="page">
      ...
      {local.curveType !== 'custom' && (
        <CurveGraph settings={graphSettings} height={200} />
      )}
      ...
      <section className="section">
        ...
        <SliderField
          label="Sensitivity"
          hint="Pointer speed. 10 = Windows default. Toggle off to restore your original OS setting."
          min={1} max={20} step={1}
          value={local.sensitivity}
          enabled={local.sensitivityEnabled}
          onToggle={v => patch({ sensitivityEnabled: v })}
