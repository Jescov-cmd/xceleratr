  ...
  return (
    <div className="cg-wrap">
      ...
      <svg viewBox={`0 0 ${W} ${H}`} className="cg-svg">
        ...
        {/* Live speed dot — always rendered; sits at curve start when idle */}
        {liveX !== undefined && (
          <circle
            cx={sx(liveX)}
            cy={sy(getCurveY(curveType, liveX, curveAcceleration, curveThreshold, curveExponent, customCurvePoints))}
            r={5}
            className="cg-live-dot"
            style={{ opacity: Math.max(0.25, Math.min(1, liveX * 8 + 0.25)) }}
          />
        )}
      </svg>
    </div>
